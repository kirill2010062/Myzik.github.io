document.addEventListener('DOMContentLoaded', () => {
    ...
    function initAuth() {
        ...
        registerForm.addEventListener('submit', (e) => {
            ...
            if (!username || !email || !password || !confirm) {
                messageEl.textContent = 'Все поля обязательны'; return;
            }
            if (username.length < 3) {
                messageEl.textContent = 'Имя должно быть не менее 3 символов'; return;
            }
            if (!email.includes('@') || !email.includes('.')) {
                messageEl.textContent = 'Введите корректный email'; return;
            }
            if (password.length < 6) {
                messageEl.textContent = 'Пароль должен быть не менее 6 символов'; return;
            }
            if (password !== confirm) {
                messageEl.textContent = 'Пароли не совпадают'; return;
            }
            if (users.find(u => u.email === email)) {
                messageEl.textContent = 'Email уже используется'; return;
            }
            
            const newUser = { id: Date.now(), username, email, password, avatar: '👤' };
            users.push(newUser);
            localStorage.setItem('freemusic_users', JSON.stringify(users));
            currentUser = { id: newUser.id, username, email, avatar: '👤' };
            localStorage.setItem('freemusic_current_user', JSON.stringify(currentUser));
    ...
    function toggleFavorite(track) {
        ...
        if (exists) {
            ...
            favorites[currentUser.id] = [...userFavs, {
                videoId: track.videoId, title: track.title, author: track.author,
    ...
    function renderFavorites() {
        ...
        container.querySelectorAll('.favorite-track-item').forEach(el => {
            el.addEventListener('click', (e) => {
                ...
                if (track) { playlist.push({...track}); renderPlaylist(); playPlaylistTrack(playlist.length - 1); }
            });
        });
        ...
        container.querySelectorAll('.remove-fav-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                localStorage.setItem('freemusic_favorites', JSON.stringify(favorites));
                renderFavorites(); updateFavoritesUI(); updateLikeButton();
            });
        });
    }
    ...
    async function searchYouTube(query, max = 20) {
        try {
            ...
            const detRes = await fetch(`https://www.googleapis.com/youtube/v3/videos?part=contentDetails&id=${ids}&key=${YOUTUBE_API_KEY}`);
    ...
    async function loadPopularTracks() {
        ...
        try { renderTracks(grid, await searchYouTube('top hits 2024', 24)); }
        catch { grid.innerHTML = '<div class="loading-placeholder">Ошибка загрузки</div>'; }
    }
    
    function renderTracks(container, videos) {
        if (!container) return;
        container.innerHTML = '';
        if (videos.length === 0) { container.innerHTML = '<div class="loading-placeholder">Ничего не найдено</div>'; return; }
        videos.forEach(v => {
            ...
            card.innerHTML = `
                ...
                <div class="track-info-text"><div>${escapeHtml(v.title)}</div><div>${escapeHtml(v.author)}</div></div>
            `;
            card.addEventListener('click', (e) => {
                if (e.target.closest('.add-to-playlist')) return;
                playlist.push(v); renderPlaylist(); playPlaylistTrack(playlist.length - 1);
            });
            card.querySelector('.add-to-playlist').addEventListener('click', (e) => {
                e.stopPropagation(); playlist.push(v); renderPlaylist();
            });
    ...
    function renderPlaylist() {
        ...
        cont.innerHTML = playlist.map((t, i) => `
            <div class="playlist-item ${i === currentIndex ? 'active' : ''}">
                <div class="item-info"><div class="item-title">${escapeHtml(t.title)}</div><div class="item-artist">${escapeHtml(t.author)}</div></div>
                <button class="remove-btn" data-index="${i}"><i class="fas fa-times"></i></button>
            </div>
        `).join('');
        cont.querySelectorAll('.playlist-item').forEach((el, i) => {
            el.addEventListener('click', (e) => { if (!e.target.closest('.remove-btn')) playPlaylistTrack(i); });
        });
        cont.querySelectorAll('.remove-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                renderPlaylist();
                if (playlist.length === 0) {
                    playerTitle.textContent = 'Не выбрано'; playerArtist.textContent = 'Добавьте трек';
                    playBtn.innerHTML = '<i class="fas fa-play"></i>';
                }
            });
        });
    ...
    function playNext() {
        if (currentIndex < playlist.length - 1) playPlaylistTrack(currentIndex + 1);
        else { isPlaying = false; playBtn.innerHTML = '<i class="fas fa-play"></i>'; currentIndex = -1; renderPlaylist(); }
    }
    ...
    function initMobilePlaylist() {
        ...
        if (!document.querySelector('.close-playlist-btn')) {
            ...
            let touchStartX = 0;
            playlistSidebar.addEventListener('touchstart', (e) => { touchStartX = e.touches[0].clientX; });
            playlistSidebar.addEventListener('touchmove', (e) => {
                if (e.touches[0].clientX - touchStartX > 50) closePlaylist();
            });
        }
    }
    ...
    document.querySelectorAll('.nav-item').forEach(item => {
        item.addEventListener('click', (e) => {
            ...
            if (item.dataset.page === 'profile') updateFavoritesUI();
        });
    });
    ...
    likeCurrentBtn.addEventListener('click', () => {
        if (currentIndex >= 0 && playlist[currentIndex]) toggleFavorite(playlist[currentIndex]);
    });
    ...
    volumeSlider.addEventListener('input', () => { if (player) player.setVolume(volumeSlider.value); });
    ...
    playBtn.addEventListener('click', () => {
        ...
        if (state === 1) player.pauseVideo();
            ...
            if (currentIndex === -1) playPlaylistTrack(0);
    ...
    document.getElementById('clear-playlist').addEventListener('click', () => {
        ...
        playerTitle.textContent = 'Не выбрано'; playerArtist.textContent = 'Добавьте трек';
        playBtn.innerHTML = '<i class="fas fa-play"></i>'; updateLikeButton();
    });
    
    if (window.innerWidth <= 768) initMobilePlaylist();
    
    window.addEventListener('resize', () => {
        if (window.innerWidth <= 768 && !document.querySelector('.close-playlist-btn')) {
            initMobilePlaylist();
